...
2. **既定値を最初にまとめてユーザーに選択させる（重要）。** 既定値を無断で採用しない。かといって
   ...
   **表示言語も必ずこの一括確認に含める。** 「多言語対応を維持するか」だけでなく、**維持しない場合の
   単一表示言語（日本語 or 英語）そのものを具体的に確定させる**こと（実例:
   PetClinic検証で「多言語対応は維持しない、日本語（または英語）」という曖昧な決定のまま
   Wave1に進んだ結果、画面ごとに`screen-designer`/`frontend-coder`が異なる言語を選び、
   ナビゲーション・見出し・ボタンが英語と日本語で混在する状態でWave1が実装完了してしまい、
   リリース判定時に人間の指摘で判明・事後修正する事態になった）。「どちらでもよい」を選択肢として
   残さず、必ずどちらか一方に確定させる。
